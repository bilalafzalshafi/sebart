...
main <- function() {
  ...
  prefix <- args$prefix %||% "real_interval_equal_tail"

  level_vec <- strsplit(args$levels, ",")[[1]]
  level_vec <- trimws(level_vec)
  ...
  saved <- save_interval_widths_by_dataset(
    ...
    levels = level_vec,
    interval_types = "Q",
    allowed_models = trimws(strsplit(args$allowed_models, ",")[[1]])
  )

  if (length(saved)) {
    message("Saved interval-width plots (equal-tail only):\n", paste(saved, collapse = "\n"))
  } else {
    message("No plots generated (check dataset filter or metrics file).")
  }
}
